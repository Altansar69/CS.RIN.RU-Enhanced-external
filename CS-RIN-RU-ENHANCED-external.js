...
function AddRinLinkToSteam() {
    if(document.location.origin.match("store.steampowered.com")) { //If you are on Steam
        ...
        // Attach element a as first child of element otherSiteInfo
        ...
        var urlToFetch = "https://cs.rin.ru/forum/search.php?keywords="+appid+"&fid%5B%5D=10&sr=topics&sf=firstpost"; // Search URL of the game page
        var urlToRedirect;
        GM_xmlhttpRequest({
            method: "GET", url: urlToFetch, onload: function (response) {
                ...
                var tags = [];
                tags = topicSelector.text.match(/\[([^\]]+)]/g).slice(1);
